---
name: webgpu-forge
description: Convert any ML model (ONNX, GGUF, TFLite) into a standalone browser project with hand-written WGSL compute shaders. No runtime dependencies, no ONNX Runtime, no transformers.js. Produces a zero-dependency TypeScript package that runs inference entirely via WebGPU. 9 phases guide you from model inspection through correctness validation to iPhone-optimized production and npm publish. Trigger words - webgpu, forge, model to webgpu, hand-written shaders, browser ml, convert model.
---
...
Given a model file (ONNX, GGUF, or TFLite), this skill guides you through generating a complete standalone repo:
...
{model-name}-webgpu/
  ...
  tsconfig.json
  CLAUDE.md            # Architecture doc for future iteration sessions
```
...
- User wants to port an existing model (PyTorch, ONNX, TFLite, GGUF) to pure WebGPU
- User mentions "hand-written shaders", "no ONNX Runtime", "zero dependencies", "browser ML"
- User references the existing projects: webgpu-gemma, kitten-tts-webgpu, micro-handpose
...
These three completed projects serve as the canonical examples. Read their source code when implementing corresponding phases:

| Project | Model Type | Source Format | Shaders | Bundle | Perf (M4 Pro) |
|---------|-----------|---------------|---------|--------|---------------|
| `~/code/webgpu-gemma/` | LLM (Gemma 3 1B) | GGUF Q8_0 | 18 | 12KB gz | 59.8 t/s |
| `~/code/kitten-tts-webgpu/` | TTS (Kitten TTS 80M) | ONNX (quantized) | 29 | 753KB gz | 3.3x RT |
| `~/code/micro-handpose/` | Vision (Hand landmarks) | TFLite | 28 | 8KB gz | 2.2ms |

Each project has full architecture documentation. Key reference files:
- `~/code/webgpu-gemma/src/` — GGUF parsing, Q8 tiled matmul, KV cache, tokenizer, conversation
- `~/code/kitten-tts-webgpu/CLAUDE.md` — ONNX pipeline architecture, 29 shader descriptions
- `~/code/micro-handpose/CLAUDE.md` — TFLite multi-model pipeline, f16 validation

---

## Phases

This skill operates in 9 sequential phases. Each phase has clear inputs, outputs, and success criteria. You can enter any phase independently — just ensure its prerequisites are met.
...
2. **Write `scripts/inspect_model.py`** — a Python script using `uv run` that:
   ...
   **Starter template for ONNX inspection:**
   ```python
   ...
   for init in graph.initializer:
       ...
       print(f"  {init.name}: {shape} {dtype} ({size_bytes:,} bytes)")
...
4. **Document findings** in the project's `CLAUDE.md` under "## Model Architecture":
...
**Key lessons from reference projects:**
...
- micro-handpose uses TWO models (palm detection + hand landmark) with NMS and affine-crop glue between them. Not all projects are single-model.
...
**Inputs:** Model file from Phase 1. Model architecture knowledge from Phase 1's `CLAUDE.md`.
...
1. **Weight extraction** — two approaches depending on source format:
   ...
   **For ONNX and TFLite: Python extraction to binary blob**
   ...
   Write `scripts/extract_weights.py` using `uv run` that:
   ...
   **For GGUF: TypeScript runtime parser (no Python extraction)**
   ...
   - **Keep embedding in quantized format on GPU** — dequantizing a 262K-token embedding to f32 uses 671MB and crashes iPhones. Use a `embeddingLookupQ8` shader that dequantizes per-token on the fly.
...
2. **Build the preprocessing pipeline** — write `src/tokenizer.ts` (or `src/phonemizer.ts`, `src/preprocessor.ts`):
   ...
   **For LLMs:**
   ...
   - Build chat template formatter (model-family specific — Gemma uses `<start_of_turn>user\n{text}<end_of_turn>\n<start_of_turn>model\n`)
...
- `CLAUDE.md` updated with weight format and preprocessing documentation
...
2. **Write `scripts/dump_activations.py`** — a Python script using `uv run` that:
   ...
   **ONNX activation dumping approach:**
   ```python
   # Key steps (see reference projects for full scripts):
   ...
   ```
   See `~/code/kitten-tts-webgpu/scripts/dump_activations.py` for a complete working example.
...
3. **Document the activation checkpoint map** in `CLAUDE.md`: which `.npy` file corresponds to which pipeline stage, with expected shapes.
...
- At least 20+ checkpoints for complex models (kitten-tts needed 37)
...
**Inputs:** Operation inventory from Phase 1. Model architecture from `CLAUDE.md`.
...
2. **Write `src/shaders.ts`** — all shaders as exported TypeScript string literals.
   ...
   **Tiled matmul template** — the workhorse shader. Use 16x16 tiles with `var<workgroup>` shared memory, `workgroupBarrier()` between load and compute phases. See `~/code/webgpu-gemma/src/shaders.ts` (matmulShader) for the canonical implementation with Q8_0 dequantization variant.
   ...
   **Two-pass reduction pattern** (required for softmax, layerNorm, instanceNorm):
   ...
   - For small vectors (hidden_size <= 4096): single-workgroup dispatch with strided loop (see fusedNormAdd in Phase 7)
   - See `~/code/webgpu-gemma/src/shaders.ts` (softmaxShader, rmsNormShader) for working implementations.
...
- **Group/depthwise convolution:** kitten-tts has `group=1090` in a ConvTranspose1d. Getting the group parameter wrong produces correct-shaped but wrong-valued output. Implement per-group weight slicing in the conv shader.
...
2. **Write `src/engine.ts`** — the forward pass orchestrator:
   ...
   **Initialization sequence** (5 steps, all at load time — zero per-inference allocation):
   ...
   See `~/code/webgpu-gemma/src/engine.ts` for the canonical LLM pattern, `~/code/kitten-tts-webgpu/src/engine.ts` for multi-stage TTS pattern.
   ...
   **Forward pass pattern — command encoder batching:**
   ...
   - Reference: kitten-tts went from 830ms → 146ms just from batching (5.7x speedup)
...
2. **Run and iterate.** The first run WILL fail. Debug using the known bug database:

   **Known Bug Database:** See `~/.claude/skills/webgpu-forge/KNOWN_BUGS.md` for the full 14-entry table (symptom → cause → fix). Covers weight layout, dequantization, varint parsing, NaN propagation, buffer flags, padding, f16 failures, and more.
...
1. **Benchmark baseline** — profile each stage individually:
   ...
   **Fused shader pattern** (fusedNormAdd — RMSNorm + residual in one dispatch):
   ...
   - See `~/code/webgpu-gemma/src/shaders.ts` (rmsNormShader) for the canonical implementation.



2. **Command encoder batching** (5-7x improvement if not done in Phase 5):
   - Reference: kitten-tts 830ms → 146ms from batching alone
...
4. **Tiled matmul with shared memory** — 2-3x for attention/FFN:
   - Reference: webgpu-gemma encoder 33ms → 16.1ms

5. **GPU migration** of remaining CPU ops:
   - Reference: kitten-tts LSTM to GPU (146→124ms), iSTFT to GPU (11.9→0.7ms)
...
**Success criteria:**
- ≥2x speedup from Phase 5 baseline
...
**Goal:** Reliable iPhone Safari (iOS 26+) via WebGPU. Every project hit iOS-specific issues.

**Inputs:** Optimized engine from Phase 7. LambdaTest account.

**Steps:**

1. **Test on LambdaTest** (see `/lambdatest` skill): iPhone 17 Pro Max, iOS 26, Safari.

   **LambdaTest workflow:**
   - Use the `/lambdatest` skill to launch a real device session
   - Deploy your demo page via `/sven-pages` or localhost tunnel
   - Open Safari DevTools (via LambdaTest remote inspector) to check console for WebGPU errors
...
1. **Configure build:** Use `tsup` (ESM, dts, minify). Zero `dependencies` — everything bundled. `devDependencies` only: tsup, typescript, playwright. See reference projects for exact tsup.config.ts and package.json.
...
6. **Build and verify:** `npm run build` must pass before publishing.

7. **Publish:** `npm publish` (or `npm publish --access public` for scoped packages)
